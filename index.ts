...
function converAstToHyperscript(ast: Element[]) {
	let ret: (() => | Text)[] = []
	ast.forEach((element) => {
		...
		// if tag is each, use the of and children to make each element from solid js
			...
			let of: Element | undefined = element.children.find((child) => child.tag === "of")
			let children: Element | undefined = element.children.find((child) => child.tag === "children" || child.tag === "as")
			...
			if (of && children && of?.value && children?.value) {
				...
				let f = of?.children.find((e) => e.tag === "expression")
				let c = children?.children.find((e) => e.tag === "expression")

				if (!f || !c) throw new Error("Invalid each block")
			...
			if (!w || !t) throw new Error("Invalid when block")
...
class Parser {
	...
	ended() {
		if (!this.char() && !this.peekNext()) return true
		else return false
	}
	...
	parseElement(): (Element | undefined) {
		let indent = 0
		let tag = ""
		let attrs = {}
		...
		if (tag === "when" || tag === "each") {
			if (tag === "each") { children = this.parseEach(indent) }
			if (tag === "when") { children = this.parseWhen(indent) }
		}
		else children = this.parseText()
		if (tag === "#each") tag = "each"
	...
	parseWhen(indent = 0) {
		let when = this.next()
		if (when === undefined) throw new Error("Invalid when [when]: WHEN needs to be an expression")
		if (typeof when !== "string") {
			if (when.type !== "expression") throw new Error("Invalid when [when]: WHEN needs to be an expression")
		} else throw new Error("Invalid when [when]: WHEN cannot be string")

		let then = this.next()
		if (then === undefined) throw new Error("Invalid when [then]: THEN needs to be an expression")
		if (typeof then == "string") {
			if (then.trim() !== "then") throw new Error("Invalid when [then]: THEN needs to be an expression")
		} else throw new Error("Invalid when [then]: THEN cannot be string")

		let value = this.next()
		if (value === undefined) throw new Error("Invalid when block")
		if (value.type !== "expression") throw new Error("Invalid when [when]: WHEN needs to be an expression")

		let next = this.next()
		if (next === undefined) { } else this.eatWhitespace()
	...
	parseEach(indent = 0) {
		let of = this.next()
		if (of === undefined) throw new Error("Invalid each [of]: OF needs to be an expression")
		if (typeof of !== "string") {
			if (of.type !== "expression") throw new Error("Invalid each [of]: OF needs to be an expression")
		} else throw new Error("Invalid each [of]: OF cannot be string")

		let ofValue = this.next()
		if (ofValue === undefined) throw new Error("invalid each of [as]: as keyword missing")
		// this needs to be string => "as"
		if (typeof ofValue === "string") {
			if (ofValue.trim() !== "as") throw new Error("invalid each of [as]: as keyword missing")
		}
		else throw new Error("ofValue is not a string")

		let asValue = this.next()
		if (asValue === undefined) throw new Error("Invalid each block")
		if (typeof asValue.value !== "function") throw new Error("Invalid d")
		let next = this.next()
		if (next === undefined) { } else this.eatWhitespace()
	...
	makeExpressionElement(value: any) {
		return {
			tag: "expression",
			children: [],
			indent: 0,
